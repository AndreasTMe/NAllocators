...
public class ArenaAllocatorTests
{
    ...
    [Fact]
    public void GivenAllocatorRequested_WhenNewIntegerBuffer_ThenCreate()
    {
        // Arrange
        using var allocator = new ArenaAllocator(16);

        // Act
        var number = allocator.New<int>(2);
        number[0].RWRef.Value = 10;
        number[1].RWRef.Value = 15;

        // Assert
        Assert.Equal(10, number[0].RORef.Value);
        Assert.Equal(15, number[1].RORef.Value);
        Assert.Equal(16, allocator.BufferSize);
